...
-- A despejar dados para tabela dbcantina.admin: ~11 rows (aproximadamente)
INSERT INTO `admin` (`id_admin`, `Nome`, `Login`, `Password`, `Tipo_admin_id_tipoadmin`, `Estado_atividade_idEstado_atividade`) VALUES
	...
	(10, 'Carlos', 'Carlos', '123', 1, 0),
	(11, 'Maria', 'Maria', '123', 3, 1),
	(12, 'jorgemini', 'Jorginho', 'Cocoxixi123_', 3, 1);
...
INSERT INTO `cartao` (`idCartao`, `cod_HEX`) VALUES
	(1, '053deb9af41dd24b07ff1dadf31b36e9'),
	(2, '216213d697416bfc96431c26cbf2aa2e'),
	(6, '4e74ad9b761a109c42f32a48af096f9f'),
	(4, '72e39d63fde06186f85d7ce1a257d649'),
	(5, 'a1782cde399aa463a9350c3584759b7d'),
	(7, 'b1c7dc4e4539c0549c028f708241101a'),
	(3, 'eba907ee79de1682434d52346b88b42d');

-- A despejar estrutura para tabela dbcantina.cartao_has_formando
CREATE TABLE IF NOT EXISTS `cartao_has_formando` (
  `idCartao` int(11) DEFAULT NULL,
  `idFormando` int(11) DEFAULT NULL,
  KEY `FK_idCartao` (`idCartao`),
  KEY `FK_idFormando` (`idFormando`),
  CONSTRAINT `FK_idCartao` FOREIGN KEY (`idCartao`) REFERENCES `cartao` (`idCartao`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `FK_idFormando` FOREIGN KEY (`idFormando`) REFERENCES `formando` (`idFormando`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- A despejar dados para tabela dbcantina.cartao_has_formando: ~1 rows (aproximadamente)
INSERT INTO `cartao_has_formando` (`idCartao`, `idFormando`) VALUES
	(1, 32062);
...
INSERT INTO `formando` (`idFormando`, `Nome`, `Email`, `NIF`, `Tipo_Residencia_idTipo_Residencia`, `Estado_atividade_idEstado_atividade`) VALUES
	(1, 'Ainda estou aqui retard', '', 123456789, 1, 1),
	...
	(20670, 'Tânia Filipa António', 'email@cencal.pt', 321654999, 0, 0),
	(31310, 'João Roberto Da Conceição Nogueira', 'email@cencal.pt', 321654994, 0, 1),
	(32062, 'Henrique Boita Ferreira Paulo', 'email@cencal.pt', 321654992, 0, 1),
	(33866, 'Leôncio Barbosa de Souza Neto', 'email@cencal.pt', 321654995, 0, 1),
	(35311, 'Ana Margarida Franco Coelho', 'email@cencal.pt', 321654988, 0, 1),
	(35653, 'Ricardo João Simões Da Silva Sabino Henriques', 'email@cencal.pt', 321654998, 0, 1),
	(35928, 'Manuel De Carreira Lopes E Marques Neves', 'email@cencal.pt', 321654996, 0, 1),
	(36071, 'Francisco Rafael Lopes Timóteo', 'email@cencal.pt', 321654991, 0, 1),
	(36073, 'Thatiane de Lourdes Zago Leite', 'email@cencal.pt', 321655000, 0, 1),
	(36235, 'Diogo Alexandre Cidrais Bugalho', 'email@cencal.pt', 321654989, 0, 1),
	(36350, 'Raquel Filipa Destapado Custódio', 'email@cencal.pt', 321654997, 0, 1),
	(36402, 'Vítor Daniel Gomes Querido', 'email@cencal.pt', 321655001, 0, 1),
	(36748, 'Adrielli Eluana de Carvalho dos Reis', 'email@cencal.pt', 321654987, 0, 1),
	(36816, 'Filipe Canoa Macela Corte-Real', 'email@cencal.pt', 321654990, 0, 1),
	(36921, 'Izabelle Lima Lisboa', 'email@cencal.pt', 321654993, 0, 1);
...
INSERT INTO `turma` (`idTurma`, `data_inicio`, `data_fim`, `descricao`, `cod_curso`) VALUES
	(1, '2023-01-02', '2024-05-30', 'Técnico de Programacao', NULL),
	...
	(13, '2022-05-02', '2023-03-31', 'asdasd', '481TI827.22');
